...
import ua.com.alevel.service.implement.ItemServiceImpl;
...
public class UnitTest {
    ...
    private static Items generateRandomItems() {
    ...
    private static Items generateSomeItems(String name, int price, Category category) {
        Items item = new Items();
        item.setName(name);
        item.setPrice(price);
        item.setCategory(category);
    ...
    public void shouldBeCreateItem() {
        Items item = generateRandomItems();
        itemService.create(item);
        Items[] items = itemService.findAll();
